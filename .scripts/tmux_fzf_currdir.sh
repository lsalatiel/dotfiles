...
selected_directory=$(find "$start_directory" -type d -not -path '*/.git*' -not -path '*/@*' -not -path '*/node_modules*' -not -path '*/testes*' -not -path '*/prog1*' -not -path '*/prog2*' 2>/dev/null | fzf +m)
